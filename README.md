...
平衡树解决了二叉查找树退化为近似链表的缺点，能够把查找时间控制在 (logn)，不过却不是最佳的，因为平衡树要求每个节点的左子树和右子树的高度差至多等于1，这个要求实在是太严了，导致每次进行插入/删除节点的时候，几乎都会破坏平衡树的第二个规则，进而我们都需要通过左旋和右旋来进行调整，使之再次成为一颗符合要求的平衡树。
...
永远不要在构造函数或者，析构函数中调用虚函数。https://blog.csdn.net/to_xidianhph_youth/article/details/39134801?utm_medium=distribute.pc_relevant_t0.none-task-blog-BlogCommendFromMachineLearnPai2-1.control&dist_request_id=66833a14-245e-4e04-8a97-28c7d19f6053&depth_1-utm_source=distribute.pc_relevant_t0.none-task-blog-BlogCommendFromMachineLearnPai2-1.control
#### （31） 纯虚函数
...
智能指针相互引用问题 https://blog.csdn.net/zhangfei5354/article/details/88940691。
#### （40） 调试程序的方法
* 通过设置断点进行调试
* 打印log进行调试
* 打印中间结果进行调试
...
```
因为使用成员初始化列表进行初始化的话，会直接使用传入参数的拷贝构造函数进行初始化，省去了一次执行传入参数的默认构造函数的过程（对于类中有其他类对象成员时），否则会调用一次传入参数的默认构造函数。所以使用成员初始化列表效率会高一些。<br>
...
* thread类和mutex类,condition_variable,unordered_map,右值引用。
* 新的智能指针 unique_ptr和shared_ptr



* 更多详见：https://blog.csdn.net/jiange_zh/article/details/79356417
...
* RIP“路由信息协议(Route Informatio Protocol)”的简写，主要传递路由信息，通过每隔30秒广播一次路由表，维护相邻路由器的位置关系，同时根据收到的路由表信息使用动态规划的方式计算自己的路由表信息。RIP是一个距离矢量路由协议,最大跳数为16跳,16跳以及超过16跳的网络则认为目标网络不可达。
...
* TCP数据包是没有边界的，会出现粘包的问题，UDP包是独立的，不会出现粘包问题。https://blog.csdn.net/weixin_41047704/article/details/85340311
...
第三种 考虑客服端a向服务端发起连接请求，但是这个请求包在网络中延迟堵塞了。然后客户端以为超时，所以重传了一个连接请求，这时候连接成功了。然后进行数据传输，连接结束。这是延迟的这个包到达了服务端，如果是两次握手，服务端就会又建立起连接，进行数据传输，但是客户端认为自己没有发起请求，所以不会接收来自服务端的数据。造成服务端的资源浪费。
...
持久连接又分为流水线连接和非流水线连接，非流水线连接要发出一个请求，只能在上一个请求完成后。
流水线连接可以连续发出请求。
...
**SSL是传输层的协议** https://www.cnblogs.com/sxiszero/p/11133747.html
建立连接使用非对称加密，传输内容使用对称加密。
建立连接时，服务端会把自己的公钥传给客户端，客户端生成一个对称密钥。用公钥加密，传给服务端。这样两边都有对称密钥，就可以传输信息了。非对称加密消耗大，对称加密消耗小。
...
* 非对称加密：需要公钥和私钥，公钥用来加密，私钥用来解密
  * 优点：安全，不怕泄漏  缺点：速度慢
  * 常用算法：RSA，ECC，DSA
#### （30） 数字证书的了解（高频）https://www.cnblogs.com/sxiszero/p/11133747.html
...
#### （31） 服务器出现大量close_wait的连接的原因以及解决方法 //未看
...
零拷贝：https://blog.csdn.net/weixin_44038332/article/details/108980796
